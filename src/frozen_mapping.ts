...
import { VoteChange as FrozenVoteChange } from "../generated/FrozenLockingVault/FrozenLockingVault";
import { VoteChangeEvent } from "../generated/schema";
import { ensureDelegation, ensureDelegator, ensureVoter } from "./mapping";

export function handleFrozenVoteChange(event: FrozenVoteChange): void {
  const id = event.transaction.hash.toHex() + "-" + event.logIndex.toString();
  const eventObject = new VoteChangeEvent(id);

  eventObject.vault = "FrozenLocking";
